...
function GM:HUDPaint()
        ...
        if ProcessCompleteTime then
                ...
                local second = ( RealTime() - ProcessStart) / ProcessCompleteTime
                surface.SetDrawColor(0,0,0,255)
                DrawCircle(radius, 10, centx, centy)
                surface.SetDrawColor( 0,0,190,255)
				if second > 1 then second = 1 end
                DrawCSegment(0, math.ceil(second*360), radius, 10,centx,centy)
        end
        local Q = Quest
        local pos = Q["Pos"]
...
function DrawCSegment(degs, dege, rad, res, CenterX, CenterY)
        ...
        if degs+dege >= (360-res) then
                DrawCircle(rad, res, CenterX, CenterY)
                return
        end
        local deg=degs
        if degs+dege<(360-res) then
...
function DrawCircle(rad,res, CenterX, CenterY)
        local verttab = {}
        local deg=0
        while deg<360 do
                local vert = {}
...
function CyclicClamp(val)
        local temp = math.floor(math.abs(val)/360)*360
        --if val==0 then return 0 end
        --if val==temp then return 360 end
        if val<0 then
                val = math.abs(val)
                val = (temp+360) - val
        else
...
function GM.MakeProgressBar(um)
	...
	ProcessCompleteTime = um:ReadFloat()
end
...
function CacheInfo(um)
	...
	local first = um:ReadBool()
	local final = um:ReadBool()
	local entid = um:ReadLong()
	local Cache = ents.GetByIndex(entid)
	if !Cache then return end
	if first then
		Cache.Resource = {}
	end
	for i=1,count do
		Cache.Resource[um:ReadString()] = um:ReadLong()
	end
	if final then
		gms_Cache(Cache)
	end
