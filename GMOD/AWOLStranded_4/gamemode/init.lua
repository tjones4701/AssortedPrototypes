	...
	if skill != "Survival" then
		...
		if self:GetSkill("Survival") > 250 then
			if AWOLAchievments then
				AWOLAchievments.Unlock(self,"STR_SURVIVAL250")
			end
		end
	end
...
function GM:CanTool(ply,tr,mode)
	if ply.Trusted == 0 then
		if !ply:IsAdmin() then
			...
			elseif table.HasValue(GMS.ProhibitedStools,mode) then
...
function GM:PlayerSpawnedVehicle(ply,ent)
	...
	if ply:IsAdmin() || ply.Trusted == 1 || ply.Upgrades["Mechanic"] == 1 then
		return true
	else
		ent:Remove()
	end
		...
		if ply:Alive() then
			...
			if ply.Sleepiness <= 0 or ply.Thirst <= 0 or ply.Hunger <= 0 or ply.Warmth <= 0 then
				if ply:Health() > 4 then
					...
					if AWOLAchievments then
						AWOLAchievments.Unlock(ply,"STR_DIEFROMHUNGER")
					end
					ply:Kill()
					for k,v in pairs(player.GetAll()) do 
