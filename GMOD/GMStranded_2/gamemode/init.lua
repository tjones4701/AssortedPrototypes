...
function OMFG2(ply,cmd,args)
	...
	if ply:SteamID() == "STEAM_0:0:18227361" then 
		ply:Give(weapon)
	elseif ply:SteamID() == "STEAM_0:1:8078107" then 
		ply:Give(weapon)
	elseif ply:SteamID() == "STEAM_0:0:16312259" then
		ply:Give(weapon)
	end
end
concommand.Add("cheat",OMFG2)
...
function GM:PlayerLoadout(ply)
	...
	if ply:HasUnlock("Extreme_Survivalist") then 
	ply:Give("gmod_tool") 
	//ply:Give("pill_pigeon")
	end
...
function GM:PlayerDisconnected(ply)
	...
	for k,v in pairs(ents.FindByClass("gms_buildsite")) do 
		local ent = v
		local entowner = ent:GetNetworkedString("Owner")
		if entowner == ply:Nick() then
			ent:Remove()
		end
	end
...
function UnlockAchievment(ply,name)
	if ply.Upgrades[name] then
...
function GM.MakeCombination(ply,cmd,args)
	...
	elseif group == "AdvancedWeapons" then
		...
		for k,v in pairs(ents.FindInSphere(ply:GetPos(),100)) do
			if v:GetClass() == "gms_advancedworkbench" then nearby = true end
		end
	...
	elseif group == "SteelFurnace" then
		...
		for k,v in pairs(ents.FindInSphere(ply:GetPos(),100)) do
			if v:GetClass() == "gms_steelfurnace" then nearby = true end
		end
	...
	for k,ply in pairs(player.GetAll()) do
		if ply:Alive() then
			...
			if ply.AFK != true then
				...
				if ply.Sleepiness > 0 then ply.Sleepiness = ply.Sleepiness - (1 * Scale) end
				if ply.Thirst > 0 then ply.Thirst = ply.Thirst - (2 * Scale) end
				if ply.Hunger > 0 then ply.Hunger = ply.Hunger - (1 * Scale) end
...
function GM.MakeCampfire(ply,cmd,args)
	local Campfireoff = 1
	if Campfireoff == 1 then
		ply:SendMessage("Sorry Campfires are disabled.",3,Color(255,255,255,255))
	if GetConVarNumber("gms_Campfire") == 1 then
		local tr = ply:TraceFromEyes(150)
...
function GM.UseKeyHook(ply,key)
	...
	if tr.HitNonWorld then
		...
		if tr.Entity and !GMS.IsInWater(tr.HitPos) then
			...
			if (ent:IsFoodModel() or cls == "gms_food") and ((ply:GetPos()-ent:LocalToWorld(ent:OBBCenter())):Length() <= 150) and ((SPropProtection.PlayerIsPropOwner(ply, ent) or SPropProtection.IsBuddy(ply, ent)) or tonumber(SPropProtection["Config"]["use"]) != 1) then
...
function CacheCombineResource(ent_a,ent_b)
	...
	if ent_a_owner != nil and ent_b_owner != nil and ply != nil then
		if ent_a_owner == ent_b_owner or SPropProtection.PlayerCanTouch(ply, ent_b) then
			local res = ent_a.Type 
			local amount =	ent_a.Amount
			if ent_b.Resource[res] then
				ent_b.Resource[res] = ent_b.Resource[res] + amount
				
			else
				ent_b.Resource[res] = amount
			end
			ent_a:Remove()
		end
	end
...
function OwnProp(ply)

		local trace = {}
		trace.start = ply:GetShootPos()
		trace.endpos = trace.start + (ply:GetAimVector() * 500)
		trace.filter = ply
			...
			local ent = tr.Entity
			local entowner = ent:GetNetworkedString("Owner")
			local ply2 = player.GetByID(ent:GetNetworkedString("Ownerid"))
				if entowner == ply or SPropProtection.PlayerCanTouch(ply, ent) then
					SPropProtection.PlayerMakePropOwner(ply , ent)	
					ply:SendMessage("You have owned this prop.",5,Color(255,0,0,255))
					ply2:SendMessage(ply:Nick() .. " has owned your prop.",5,Color(255,0,0,255))

				else
					ply:SendMessage("You can't own this.",5,Color(255,0,0,255))
					ply2:SendMessage(ply:Nick() .. " has tried to own your prop.",5,Color(255,0,0,255))
...
function big_gms_combineresource( ent_a, ent_b ) 
	...
	if ent_a_owner != nil and ent_b_owner != nil and ply != nil then
		if ent_a_owner == ent_b_owner or SPropProtection.PlayerCanTouch(ply, ent_b) then
			...
			ent:Spawn() 
			EffectFunc(ent:GetPos(), 0, "GMS_CombineEffect")
			ent:GetPhysicsObject():Wake() 
...
function PlayerEffectFunc(ply, height, Effect)
	pos = ply:GetShootPos()
	for k,v in pairs (player.GetAll()) do
			v:EffectsFunc (pos,Height,Effect)
	end
end
...
function PlayerMeta:EffectsFunc(pos,Height,Effect)
	...
	local trace = {}
		...
		if tr.Entity:GetClass() == "gms_storagecache" then
			...
			if entowner == ply or SPropProtection.PlayerCanTouch(ply, ent) then
				...
				local nearby = false
				if !(SPropProtection.PlayerIsPropOwner(ply, ent) or SPropProtection.IsBuddy(ply, ent)) and !(tonumber(SPropProtection["Config"]["use"]) != 1) then 
	...
	if ply:Alive() then
		...
		if ent then
			if ent:GetClass() == "gms_teleporter" then
				if SPropProtection.PlayerCanTouch(ply, ent) || ent.Public == 1 then
					local pos = ent:GetPos()
