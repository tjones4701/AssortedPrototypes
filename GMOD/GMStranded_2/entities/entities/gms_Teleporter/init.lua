...
function ENT:Use(ply)
	if self.Entity.Time < CurTime() then
		if SPropProtection.PlayerCanTouch(ply, self.Entity) || self.Entity.Public == 1 then
			local tele = {}
			local telename = {}
			local teleowner = {}
			local amount = 0
			for k,v in pairs (ents.FindByClass("gms_Teleporter")) do
				if SPropProtection.PlayerCanTouch(ply, v) || v.Public == 1 then
					if v.TeleName && (v != self.Entity) then
						amount = amount + 1
						tele[amount] = v
						telename[amount] = v.TeleName
						teleowner[amount] = v:GetNetworkedString("Owner")
					end
				end
			end
