...
local MAX_HEALTH = 25
local BASE_DAMAGE = 0 -- How much damage to do on average per attack
local EXPLODE_DAMAGE = 0 -- Maximum damage to deal when exploding on death
local EXPLODE_RADIUS = 0
local JUMP_VERT_SPEED = 180
local JUMP_HORIZ_SPEED = 300
local WADDLE_SPEED = 100
local TARGET_RADIUS = 0 -- The chicken only attacks entities if they are at least this close to it
...
function ENT:OnTakeDamage( dmginfo )
	...
	if dmginfo:GetAttacker():IsPlayer() then
		self.Entity.Owner = dmginfo:GetAttacker()
	end
...
function ENT:DoDeath()
	...
	local chicken = ents.Create("gms_loot")
			...
			if self.Entity.Owner then
				SPropProtection.PlayerMakePropOwner(self.Entity.Owner , chicken)
			end
			res["Meat"] = num
			res["Feathers"] = num
			tbl.Resources = res
			...
			chicken:Spawn()
			timer.Simple(180, function(chicken) if chicken then chicken:Remove() end end, chicken)
