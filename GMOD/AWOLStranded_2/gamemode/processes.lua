...
function PlayerMeta:DoProcess(name,time,data)
		local ply = self
		 ...
		 local oldtime = time
		 if self.Upgrades["Half_Process_Time"] == 1 then
			time = oldtime / 2
		 end
		 
		 if self.Upgrades["Quarter_Process_Time"] == 1 then
			time = oldtime / 4
		 end
...
function PROCESS:OnStop()
	if self.Owner:Alive() then
         ...
         if num < self.Data.Chance + self.Owner:GetSkill("Mining") then
			...
			if self.Owner:GetActiveWeapon():GetClass() == "gms_ironpickaxe" && self.Owner:GetSkill("Mining") > 50 then 
				num2 = math.random(1,6)
			end
			
			if self.Owner:GetActiveWeapon():GetClass() == "gms_steelpickaxe" then 
				num2 = math.random(1,8)
			end
			
			if self.Owner:GetActiveWeapon():GetClass() == "gms_diamondpickaxe" then 
				num2 = math.random(3,8)
			end
			
			if self.Owner:GetActiveWeapon():GetClass() == "gms_refined_diamondpickaxe" then 
				num2 = math.random(4,11)
			end
			if self.Owner:GetActiveWeapon():GetClass() == "gms_cheatpickaxe" then 
				num2 = math.random(4,10)
			end
			
            local num3 = math.random(self.Data.MinAmount,self.Data.MaxAmount)
			local CoalAmount = math.floor((num3/2))
			if CoalAmount < 1 then
				CoalAmount = 1
			end
			...
			elseif num2 == 5 then
               ...
               self.Owner:SendMessage("Iron Ore ("..num3.."x)", 3, Color(10,200,10,255))
			elseif num2 == 6 then
               self.Owner:IncResource("Coal",num3)
               self.Owner:SendMessage("Coal ("..num3.."x)", 3, Color(10,200,10,255))
			elseif num2 == 7 then
               self.Owner:IncResource("Coal",num3)
               self.Owner:SendMessage("Coal ("..num3.."x)", 3, Color(10,200,10,255))
			 elseif num2 == 8 then
               self.Owner:IncResource("Diamond",num3)
               self.Owner:SendMessage("Diamond ("..num3.."x)", 3, Color(10,200,10,255))
			 elseif num2 == 9 then
               self.Owner:IncResource("Copper",num3)
               self.Owner:SendMessage("Copper ("..num3.."x)", 3, Color(10,200,10,255))
			 elseif num2 == 10 then
...
function PROCESS:OnStop()
         ...
         if num > 50 - self.Owner:GetSkill("Harvesting") - add then
            ...
            self.Owner:EmitSound(Sound("items/ammo_pickup.wav"))
			local Leaves = math.random(0,5)
				self.Owner:IncResource("Leaves",Leaves)
				if Leaves > 0 then
					self.Owner:SendMessage("Leaves ("..Leaves.."x)", 3, Color(10,200,10,255))
				else
...
function PROCESS:OnStop()
		 ...
		 local isupgraded = 0
		 if self.Owner.Upgrades["InstantPlant"] == 1 then
			isupgraded = 1
		 end
         tbl:Setup("melon",160 - math.Clamp(self.Owner:GetSkill("Planting"),0,60) + math.random(-20,20),self.Owner,isupgraded)
         ent:Spawn()
		if AWOLAchievments then
			AWOLAchievments.AddCount(self.Owner,"STR_PLANT100",1)
		end
...
function PROCESS:OnStop()
	...
	if num < self.Data.Chance + self.Owner:GetSkill("Fishing") then
		...
		if self.Owner:GetSkill("Fishing") > 15 then
			max = 6
		end
		if self.Owner:GetSkill("Fishing") > 20 then
			max = 7
		end
		if self.Owner:GetSkill("Fishing") > 25 then
			max = 8
		end
		if self.Owner:GetSkill("Fishing") > 30 then
			max = 9
		end
		
		local fish = math.random(1,4)
		if fish < 4 then
	...
	self.Owner:Freeze(false)
	if AWOLAchievments then
		AWOLAchievments.AddCount(self.Owner,"STR_CATCHFISH100",1)
	end
end
...
function PROCESS:OnStop()
		 ...
		 if AWOLAchievments then
			if self.Data.Name == "Pistol" then
				AWOLAchievments.Unlock(self.Owner,"STR_MAKEPISTOL")
			end
			AWOLAchievments.AddCount(self.Owner,"STR_WEAPONMAKE500",1)
		 end
