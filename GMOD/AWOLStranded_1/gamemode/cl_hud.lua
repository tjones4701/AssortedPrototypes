...
function DrawHud()
	...
	//Main
		...
		//FrontStats
		local H = math.Clamp((LocalPlayer():Health() / 200) * 160,0,160)
	...
	if tobool(gms_CheckSettings("DrawClock")) == true then
		...
		local y = 15
		Radius = w / 2.5
		draw.RoundedBox( 6, x - 5, y - 5, w + 10, h + 10, Color( 0, 0, 0, 150 ) )
		draw.RoundedBox( 6, x, y, w, h, Color( 255, 255, 255, 150 ) )
		local centx = x + w / 2
		local centy = y + h / 2
		draw.RoundedBox( 1, centx, centy, 4, 4, Color( 0, 0, 0, 255 ) )
		w = w / 10
		h = h / 10
		x = centx - (math.sin(math.rad(-second * 6)) * Radius)
		y = centy- (math.cos(math.rad(second * 6)) * Radius)
		draw.RoundedBox( 4, x - (w / 2), y - (h / 2), w, h, Color( 255, 0, 0, 255 ) )
		x = centx - (math.sin(math.rad(-minute * 6)) * Radius)
		y = centy- (math.cos(math.rad(minute * 6)) * Radius)
		draw.RoundedBox( 4, x - (w / 2), y - (h / 2), w, h, Color( 0, 255, 0, 255 ) )
		x = centx - (math.sin(math.rad(-hour * 30 )) * Radius)
		y = centy- (math.cos(math.rad(hour * 30)) * Radius)
		draw.RoundedBox( 4, x - (w / 2), y - (h / 2), w, h, Color( 0, 0, 255, 255 ) )
		for i = 1,12 do 
			x = centx - (math.sin(math.rad(-i * 30 )) * Radius)
			y = centy- (math.cos(math.rad(i * 30)) * Radius)
			draw.SimpleText(i,"ClockFont_16",x, y, Color(255,255,255,255),1,1)
		end
		draw.SimpleText(hour..":"..minute..":"..second.." "..timeframe,"ClockFont_20",centx, centy - 10, Color(255,255,255,255),1,1)
	end
end
