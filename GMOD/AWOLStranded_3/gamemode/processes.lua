...
function PROCESS:OnStop()
         ...
         ent:Spawn()
		if self.Owner.AWOLAddCount then
			self.Owner:AWOLAddCount("STR_PLANT100",1)
		end
...
function PROCESS:OnStop()
	...
	self.Owner:Freeze(false)
	if self.Owner.AWOLAddCount then
		self.Owner:AWOLAddCount("STR_CATCHFISH100",1)
	end
end
...
function PROCESS:OnStop()
	...
	if self.Owner.AWOLGiveAchievment then
		if self.Data.Name == "Pistol" then
			self.Owner:AWOLGiveAchievment("STR_MAKEPISTOL")
		end
		self.Owner:AWOLAddCount("STR_WEAPONMAKE500",1)
	end
