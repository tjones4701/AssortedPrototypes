...
function ENT:Setup(strType,time,ply,upgraded)
...
function ENT:Grow()
	...
	elseif strType == "melon" then
		...
		num = math.floor((ply:GetSkill("Planting") / 12)) or 0
		if num > 8 then
			num = 8
		elseif num < 1 then
			num = 1
...
function GM.MakeTree(pos)
	local entitys = ents.FindInSphere( pos, 10 )
