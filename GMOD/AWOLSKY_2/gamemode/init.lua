...
AddCSLuaFile( "trophycombinations.lua" )
...
include( 'trophycombinations.lua' )
--Spawnmenu
...
concommand.Add("learned_weapon",OMFG2)
...
function PlayerMeta:IncSkill(skill,int)
	...
	if skill != "Survival" then
		...
		if self:GetSkill("Survival") >= 1000 then
				if self.AWOLGiveAchievment then
					self:AWOLGiveAchievment("CNS_SURVIVAL1000")
				end
		elseif self:GetSkill("Survival") >= 500 then
				if self.AWOLGiveAchievment then
					self:AWOLGiveAchievment("CNS_SURVIVAL500")
				end
		elseif self:GetSkill("Survival") >= 250 then
				if self.AWOLGiveAchievment then
					self:AWOLGiveAchievment("CNS_SURVIVAL250")
				end
		end
	end
...
ResourceModel = {}
ResourceModel["Compound11"] = "models/props_wasteland/rockgranite03c.mdl"
ResourceModel["Stone"] = "models/props_wasteland/rockgranite03c.mdl"
ResourceModel["Unobtainium_Ore"] = "models/props_wasteland/rockgranite03c.mdl"
ResourceModel["Platinum_Ore"] = "models/props_wasteland/rockgranite03c.mdl"
ResourceModel["Crystal"] = "models/props_wasteland/rockgranite03c.mdl"
ResourceModel["Wood"] = "models/gmstranded/props/lumber.mdl"
ResourceModel["Acidic_Water"] = "models/props_junk/garbage_plasticbottle001a.mdl"
ResourceModel["Water_Bottles"] = "models/props_junk/garbage_plasticbottle001a.mdl"
ResourceModel["Bass"] = "models/props/cs_militia/fishriver01.mdl"
ResourceModel["Shark"] = "models/props/cs_militia/fishriver01.mdl"
ResourceModel["Trout"] = "models/props/cs_militia/fishriver01.mdl"
ResourceModel["Salmon"] = "models/props/cs_militia/fishriver01.mdl"
ResourceModel["Gold_Nugget"] = "models/props_junk/PopCan01a.mdl"


ResourceMat = {}
ResourceMat["Compound11"] = "models/props_pipes/pipemetal001a"
ResourceMat["Unobtainium_Ore"] = "models/weapons/v_stunbaton/w_shaft01a"
ResourceMat["Platinum_Ore"] =	"models/props_canal/rock_riverbed01a"
ResourceMat["Platinum"] = "models/props/cs_office/snowmana"
ResourceMat["Unobtainium"] = "models/screenspace"
ResourceMat["Adamantium"] = "models/props_debris/metalwall001a"
ResourceMat["Concrete"] = "models/props_c17/metalladder001"
ResourceMat["Glass"] = "models/props_combine/health_charger_glass"
ResourceMat["Diamond"] = "models/props/cs_office/clouds"
ResourceMat["Crystal"] = "models/props_c17/frostedglass_01a_dx60"
ResourceMat["Acidic_Water"] = "models/shadertest/shader4"
ResourceMat["Flesh"] = "models/flesh"
ResourceMat["Gold_Nugget"] = "models/props/cs_assault/pylon"
ResourceMat["Gold"] = "models/props/cs_assault/pylon"
...
function PlayerMeta:Heal(int)
	local max = 100
	if self.Upgrades["Genetic_Perfection"] == 1 then
		max = 400
	elseif self.Upgrades["Genetic_Modification"] == 1 then
		max = 300 
	elseif self.Upgrades["Altered"] == 1 then
		max = 200
	end
	if self:Health() > max then return end
	self:SetHealth(math.Clamp(self:Health() + int,0,max))
end
...
function EntityMeta:IsStargateModel()
	local model = self:GetModel()
	for k,v in pairs(GMS.StargateModels) do
		if string.lower(v) == model or string.gsub(string.lower(v),"/","\\") == model then
			return true
		end
	end
...
function GM:PlayerInitialSpawn(ply)
	...
	local num = tonumber((BithLoad("GMStranded/Saves/"..ply:UniqueID().."/learned.txt") or 0 ))
	ply.learned = num
	Savelearned(ply)
end
...
function GM:PlayerSpawn(ply)
	...
	if ply.Upgrades["Speed_Demon"] == 1 then
		ply:SetWalkSpeed(600)
		ply:SetRunSpeed(250)
	
	elseif ply.Upgrades["Sprinter"] == 1 then
		ply:SetWalkSpeed(500)
		ply:SetRunSpeed(250)

	elseif ply.Upgrades["Speed"] == 1 then
		ply:SetWalkSpeed(250)
		ply:SetRunSpeed(500)

	elseif ply.Upgrades["Sprint Man"] == 1 then
		ply:SetWalkSpeed(250)
		ply:SetRunSpeed(400)	
	else
		ply:SetWalkSpeed(250)
	...
	if ply.Upgrades["PlatinumAge"] == 1 then
		ply:Give("gms_platinumhatchet")
		ply:Give("gms_platinumpickaxe")
	end
	if ply.Upgrades["UnobtainiumAge"] == 1 then
		ply:Give("gms_unobtainiumhatchet")
		ply:Give("gms_unobtainiumpickaxe")
	end
	...
	if ply.Upgrades["RedRibbon"] == 0 then 
		if ply:GetModel() == "models/player/sam.mdl" then 
			ply:SetModel("models/player/Kleiner.mdl") 
			ply:SendMessage("This model requires you to have the 'RedRibbon' trophy.\n", 3, Color(10,200,10,255))
		end
	end
	
	if ply.Upgrades["OldBoba"] == 0 then
		if ply:GetModel() == "models/bobafett_tfu.mdl" then
			ply:SetModel("models/player/Kleiner.mdl")
			ply:SendMessage("This model requires you to have the 'OldBoba' trophy", 3, Color(19,200,10,255))
		end
	end
	
	if ply.Upgrades["CombineInvasion"] == 0 then
		if ply:GetModel() == "models/player/neo_heavy.mdl" then
			ply:SetModel("models/player/Kleiner.mdl")
			ply:SendMessage("This model requires you to have the 'CombineInvasion' trophy", 3, Color(19,200,10,255))
		end
	end
	
	if !ply:IsSuperAdmin() then
		if ply:GetModel() == "models/player/slow/fallout_3/power_armor/slow.mdl" then
			ply:SetModel("models/player/Kleiner.mdl")
			ply:SendMessage("You do not have the privileges to use this model.\n", 3, Color(10,200,10,255))
		end
	end
	...
	if ply.Upgrades["Genetic_Perfection"] == 1 then
		ply:SetHealth(400) 

	elseif ply.Upgrades["Genetic_Modification"] == 1 then
		ply:SetHealth(300) 
	
	elseif ply.Upgrades["Altered"] == 1 then
		ply:SetHealth(200) 
	end
	ply:UpdateNeeds()
	SendAchievments(ply)
...
function GM:PlayerLoadout(ply)
	...
	if ply.Upgrades["Technician"] == 1 then
		ply:Give("gmod_tool") 
	end
	if ply.Upgrades["Healer"] == 1 then
		ply:Give("gms_healer") 
	end
	
	if ply.Upgrades["SnowBalls"] == 1 and ply.learned == 1 then
		ply:Give("real_snowball_swep") 
	end
	
	if ply.Upgrades["Shovel"] == 1 and ply.learned == 1 then
		ply:Give("gms_shovel") 
	end
	
	if ply.Upgrades["Fun And Games Sniper"] == 1 then
		ply:Give("weapon_crossbow")
		if ply.AWOLGiveAchievment then
				ply:AWOLGiveAchievment("CNS_FUNANDGAMESSNIPER")
		end
	end
	
	if ply.Upgrades["Fun And Games Melee"] == 1 then
		ply:Give("weapon_crowbar")
		if ply.AWOLGiveAchievment then
				ply:AWOLGiveAchievment("CNS_FUNANDGAMESMELEE")
		end
	end
	
	if ply.Upgrades["Fun And Games Ranged"] == 1 then
		ply:Give("weapon_smg1")
		if ply.AWOLGiveAchievment then
				ply:AWOLGiveAchievment("CNS_FUNANDGAMESRANGED")
		end
	end
		...
		if (tr.MatType == MAT_DIRT or tr.MatType == MAT_GRASS or tr.MatType == MAT_SAND) and !GMS.IsInWater(tr.HitPos) then
			if ply:GetResource("Melon_Seeds") >= 1 then
				if !GMS.ClassIsNearby(tr.HitPos,"gms_seed",30) and !GMS.ClassIsNearby(tr.HitPos,"prop_physics",50) then
					...
					data.Pos = tr.HitPos
					local time = 3
					if ply.Upgrades["Advanced_Fertilizer"] == 1 then
						time = .1
					end
...
function GM.PlantTree(ply,cmd,args)
	...
	if tr.HitWorld then 
		if (tr.MatType == MAT_DIRT or tr.MatType == MAT_GRASS or tr.MatType == MAT_SAND) and !GMS.IsInWater(tr.HitPos) then
			if ply:GetResource("Tree_Seedlings") >= 1 then
				local data = {}
				data.Pos = tr.HitPos
				local time = 3
					if ply.Upgrades["Advanced_Fertilizer"] == 1 then
				...
				ply:SendMessage("You need a Tree Seedling.",3,Color(200,0,0,255))
			end
		else
			ply:SendMessage("You cannot plant on this terrain.",3,Color(200,0,0,255))
		end
...
function GM.MakeCombination(ply,cmd,args)
	...
	local struct = ply:GetSkill("Construction")
	...
	elseif group == "PlatinumWeapons" then
		local nearby = false

		for k,v in pairs(ents.FindInSphere(ply:GetPos(),100)) do
			if v:GetClass() == "gms_platinumworkbench" and v:CPPICanUse(ply) then nearby = true end
		end
	...
	elseif group == "UnobtainiumWeapons" then
		local nearby = false

		for k,v in pairs(ents.FindInSphere(ply:GetPos(),100)) do
			if v:GetClass() == "gms_unobtainiumworkbench" and v:CPPICanUse(ply) then nearby = true end
		end
	...
	elseif group == "PlatinumFurnace" then
		local nearby = false

		for k,v in pairs(ents.FindInSphere(ply:GetPos(),100)) do
			if v:GetClass() == "gms_platinumfurnace" and v:CPPICanUse(ply) then nearby = true end
		end
	...
	elseif group == "UnobtainiumFurnace" then
		local nearby = false

		for k,v in pairs(ents.FindInSphere(ply:GetPos(),100)) do
			if v:GetClass() == "gms_unobtainiumfurnace" and v:CPPICanUse(ply) then nearby = true end
		end
	...
	elseif group == "AdvancedFurnace" then
		local nearby = false

		for k,v in pairs(ents.FindInSphere(ply:GetPos(),100)) do
			if v:GetClass() == "gms_advancedfurnace" and v:CPPICanUse(ply) then nearby = true end
		end
	...
	elseif group == "TradingCart" then
		local nearby = false

		for k,v in pairs(ents.FindInSphere(ply:GetPos(),100)) do
			if v:GetClass() == "gms_tradingcart" and v:CPPICanUse(ply) then nearby = true end
		end

		if !nearby then
			ply:SendMessage("You need to be close to a the Trading Cart!",3,Color(200,0,0,255))
			ply:CloseCombiMenu()
		return end
	end		 
	...
	if group == "Cooking" then
		...
		if ply:GetActiveWeapon():GetClass() == "gms_adamantiumfryingpan" then
			time = 1
		end
	...
	elseif group == "Factory" then
		...
		local Amount2 = 0
		if Compare > 0 && Compare < 50 then
			Amount2 = Amount / 6
		elseif Compare > 50 && Compare < 100 then
			Amount2 = Amount / 7
		elseif Compare > 100 && Compare < 500 then
			Amount2 = Amount / 8
		elseif Compare > 500 && Compare < 1500 then
			Amount2 = Amount / 9
		elseif Compare > 1500 && Compare < 2500 then
			Amount2 = Amount / 10
		elseif Compare > 2500 && Compare < 5000 then
			Amount2 = Amount / 11
		elseif Compare > 5000 then
			Amount2 = Amount / 12
		else
			Amount2 = Amount
		end
	...
	elseif group == "Buildings" then
		...
		local time = 10
		if struct >= 0 and struct < 20 then
			time = 16
		elseif struct >= 20 and struct < 50 then
			time = 12
		elseif struct >= 50 and struct < 100 then
			time = 8
		elseif struct >= 100 and struct < 200 then
			time = 6
		elseif struct >= 200 and struct < 500 then
			time = 4
		elseif struct >= 500 then
			time = 2
		end
	...
	elseif group == "StoneFurnace" then
		...
		elseif Compare > 50 && Compare < 100 then
			Amount2 = Amount / 3
		elseif Compare > 100 && Compare < 300 then
			Amount2 = Amount / 4
		elseif Compare > 300 && Compare < 500 then
			Amount2 = Amount / 5
		elseif Compare > 500 && Compare < 800 then
			Amount2 = Amount / 6
		elseif Compare > 800 && Compare < 1500 then
			Amount2 = Amount / 7
		elseif Compare > 1500 then
			Amount2 = Amount / 8
		else
			Amount2 = Amount
		end
	...
	elseif group == "TradingCart" then
		...
		local Amount = 0
		for k, v in pairs(data.Cost) do
			local TimeOff = ply:GetSkill("Weapon_Crafting") or 0
			TimeOff = v - TimeOff
			TimeOff = math.floor(TimeOff)
			if TimeOff < 1 then
				TimeOff = 1
		...
		local time = timecount * 0.5
		local Compare = ply:GetSkill("Weapon_Crafting") or 0
		local Amount2 = 0
		if Compare > 0 && Compare < 10 then
			Amount2 = Amount / 2
		elseif Compare > 10 && Compare < 20 then
			Amount2 = Amount / 3
		elseif Compare > 20 && Compare < 30 then
			Amount2 = Amount / 4
		elseif Compare > 30 && Compare < 40 then
			Amount2 = Amount / 5
		elseif Compare > 40 && Compare < 50 then
			Amount2 = Amount / 6
		elseif Compare > 50 && Compare < 60 then
			Amount2 = Amount / 7
		elseif Compare > 60 && Compare < 70 then
			Amount2 = Amount / 8
		elseif Compare > 70 && Compare < 80 then
			Amount2 = Amount / 9
		elseif Compare > 80 && Compare < 90 then
			Amount2 = Amount / 10
		elseif Compare > 99 then
			Amount2 = Amount / 11
		else
			Amount2 = Amount
		end
...
function GM:PhysgunPickup(ply, ent)
	...
	if ent.StrandedProtected || ent:IsPlayer() || (ent:IsStargateModel() or table.HasValue(GMS.PickupProhibitedClasses,ent:GetClass())) then
		return false
	else
		return true
	end
...
function GM:CanTool(ply,tr,mode)
	if ply:IsAdmin() then return true end
	local ent = tr.Entity
		if IsValid(ent) && ent:IsVehicle() && ply.learned == 1 then
			if mode == "weld" then
				return true
			...
			elseif mode == "remover" then
				return true
			elseif mode == "hoverball" then
				return true
			elseif mode == "thruster" then
				return true
			elseif mode == "rope" then
				return true
			elseif mode == "keep upright" then
				return true
			elseif mode == "camera" then
				return true
			elseif mode == "rt camera" then
				return true
			elseif mode == "material" then
				return true
			elseif mode == "trails" then
				return true
			else
				ply:SendMessage("This sTOOL is prohibited on vehicles.",3,Color(200,0,0,255))
				return false
			end
		end
		
		if IsValid(ent) && ent:IsVehicle() && ply.learned == 0 then
			ply:SendMessage("This Stool is prohibited on vehicles.",3,Color(200,0,0,255))
			return false
		end
		
	if mode =="stargate_cloaking" then
		if ply.Upgrades["Cloak"] == 1 then
			return true
		else
			return false
		end
	elseif mode == "stargate_shield" then
		if ply.Upgrades["Shield"] == 1 then
			return true
		else 
			return false
		end
	elseif mode == "stargate_iris" then
		if ply.Upgrades["Iris"] == 1 then
			return true
		else 
			return false
		end
	elseif mode == "door" then
		if ply.Upgrades["Door"] == 1 then
			return true
		else 
			return false
		end
	...
	if table.HasValue(GMS.ProhibitedStools,mode) then
		if ply.learned == 1 then
			if table.HasValue(GMS.learnedAllowedTools,mode) then
				return true
			end
		end
...
function GM:PlayerSpawnedProp( ply, mdl, ent )
	...
	if ply.CanSpawnProp == false and !ply:IsAdmin() then
		ent:Remove()
...
function GM:PlayerSpawnedPropDelay( ply, mdl, ent )
	...
	if ply.Upgrades["Compressed_Resources"] == 1 then
		cost = cost / 2
	end
	...
	for k,ply in pairs(player.GetAll()) do
		if ply:Alive() then
			...
			if ply.Sleeping then
				if ply.Sleepiness < 1000 and ply.Sleepiness <= 950 then
					...
					if Entity(tr.Entity) and tr.Entity:IsSleepingFurniture() then
						if ply.Upgrades["Lucid Dreaming"] == 1 then
							ply.Sleepiness = ply.Sleepiness + 275
						elseif ply.Upgrades["Good Sleeper"] == 1 then
							ply.Sleepiness = ply.Sleepiness + 150
						else
			...
			if ply.AFK != true then
				...
				//end
				if ply.Upgrades["Holy_Grail"] == 1 then
					Scale = .25
				elseif ply.Upgrades["Strong_Metabolism"] == 1 then
					Scale = .5
				else
					Scale = 1
				end
	...
	if tr.HitNonWorld then
		...
		if tr.Entity and !GMS.IsInWater(tr.HitPos) then
			...
			elseif ent:IsTreeModel() then
				ply:DoProcess("Tree_SeedlingsCollect",5)
...
/*---------------------------------------------------------
  Tribe: The Enterprise
---------------------------------------------------------*/
function GM.Tribe1( ply )
	ply:SetTeam(1)
	ply:SendMessage("Successfully changed to Tribe: The Enterprise!",5,Color(255,255,255,255))
end

concommand.Add("gms_tribe1", GM.Tribe1)
/*---------------------------------------------------------
  Tribe: Sky Scavengers
---------------------------------------------------------*/
function GM.Tribe2( ply )
	ply:SetTeam(24)
	ply:SendMessage("Successfully changed to Tribe: Sky Scavengers",5,Color(255,255,255,255))
end

concommand.Add("gms_tribe2", GM.Tribe2)
/*---------------------------------------------------------
  Tribe: Space Invaders
---------------------------------------------------------*/
function GM.Tribe3( ply )
	ply:SetTeam(23)
	ply:SendMessage("Successfully changed to Tribe: Space Invaders",5,Color(255,255,255,255))
end
...
function GM.Tribe4( ply )
	ply:SetTeam(22)
	ply:SendMessage("Successfully changed to Tribe: Sky Bandits",5,Color(255,255,255,255))
end
...
function GM.Tribe5( ply )
	ply:SetTeam(21)
	ply:SendMessage("Successfully changed to Tribe: Land Lovers",5,Color(255,255,255,255))
end
...
function GM.Tribe6( ply )
	ply:SetTeam(20)
	ply:SendMessage("Successfully changed to Tribe: Space Ninjas",5,Color(255,255,255,255))
end
			...
			if ent:CPPICanUse(ply) then
				...
				if nearby == true && entity != ply then
						if entity.Resource[Res] then
							...
							--HotFix, Because I'm lazy. SirWaddles
							if entity.Resource[Res]<=0 then
								entity.Resource[Res] = nil
							end
						else
...
function SendCacheInfo(ply, Cache)
	//This function was FINALLY FIXED AHAHA - SirWaddles
	
	if !Cache or Cache:GetPos():Distance(ply:GetPos()) > 300 then
		ply:SendMessage("Must be near a cache", 3, Color(10,200,10,255))
		return
	end
	local SendRes = {}
	for k,v in pairs(Cache.Resource) do
		local entr = {}
		entr.Name = k
		entr.Count = v
		table.insert(SendRes, entr)
	end
	
	local count = table.Count(SendRes)
		...
		if ValidEntity(ent) then
			if ent:CPPICanUse(ply) then
				if ent.CurrentCapacity > 0 then
					...
					ply:SendMessage("You have emptied "..ent.CurrentCapacity.." "..ent.Res..".",3,Color(200,0,0,255))
	...
	if args[1] then
		...
		if ValidEntity(ent) then
			if ent:CPPICanUse(ply) then
				...
				if ply.Resources["Compound11"] >= amount then
					ply:DecResource("Compound11",amount)
					ent.Fuel = ent.MaxFuel
					ply:SendMessage("You have refilled the tank.",3,Color(200,0,0,255))
				elseif ply.Resources["Compound11"] > ply.Resources["Compound11"] then
					ply:DecResource("Compound11",ply.Resources["Compound11"])
					ent.Fuel = ent.Fuel + ply.Resources["Compound11"]
					ply:SendMessage("You have partially filled the tank.",3,Color(200,0,0,255))
				else
					ply:SendMessage("You don't have any Compound11 to refill the tank.",3,Color(200,0,0,255))
				end
			else
				ply:SendMessage("You can't touch this.",3,Color(200,0,0,255))
			end
...
function MakePlayerlearned(ply,cmd,args)
	if ply:IsAdmin() then
		local name = args[1]
		for k,v in pairs (player.GetAll()) do
			if string.find(string.lower(v:Nick()),string.lower(name)) then
				...
				if v.learned == 0 then
					v.learned = 1
					ply:SendMessage("learned given to "..v:Nick(),10,Color(255,255,0,255))
					v:SendMessage("You have recieved learned from "..ply:Nick().. " don't abuse it!",10,Color(255,255,0,255))
				else
					v.learned = 0
					ply:SendMessage("learned removed from "..v:Nick(),10,Color(255,255,0,255))
					v:SendMessage("You have recieved learned from "..ply:Nick().. " don't abuse it!",10,Color(255,255,0,255))
				end
			else
				print("No player")
...
concommand.Add("learned_amakelearned",MakePlayerlearned)
	...
	if ply:IsAdmin() then
		...
		for k,v in pairs (player.GetAll()) do
			if string.find(string.lower(v:Nick()),string.lower(name)) then
				...
				if ply:IsAdmin() then
					ply:SendMessage("Can't kick admin or learned!",10,Color(255,255,0,255))
				else
...
concommand.Add("learned_kick",gmskick)

function Savelearned(ply)
	if ply.learned == nil then
		local num = tonumber((BithLoad("GMStranded/Saves/"..ply:UniqueID().."/learned.txt") or 0 ))
		ply.learned = num
	end
	if ply:IsAdmin() then
	...
	BithSave("GMStranded/Saves/"..ply:UniqueID().."/learned.txt",ply.learned)
end

concommand.Add("learned_loadme",Savelearned)
